...
// Query JOIN untuk Laporan (Syarat UAS No. 2)
...
<div class="d-flex">
    ...
    <div class="flex-grow-1 p-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-semibold text-dark">Laporan Pembelian</h2>
            <a href="tambah_pembelian.php" class="btn btn-primary px-4 py-2 shadow-sm rounded-pill">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-3 fw-bold text-primary"><?= $row['no_invoice']; ?></td>
                            <td><?= $row['username']; ?></td>
                            <td><?= date('d M Y, H:i', strtotime($row['tanggal_beli'])); ?></td>
                            <td class="fw-bold text-success">Rp <?= number_format($row['total_bayar'], 0, ',', '.'); ?></td>
                            <td class="text-center">
                                <a href="edit_pembelian.php?id=<?= $row['id_beli']; ?>" class="btn btn-sm btn-warning text-white rounded-3 shadow-sm me-1">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="hapus_pembelian.php?id=<?= $row['id_beli']; ?>" class="btn btn-sm btn-danger rounded-3 shadow-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus transaksi ini?')">
